...
export const EmptyState = ({
  ...
  const artwork = illustration ?? (icon === 'cloud-offline-outline' ? 'offline' : undefined);
  return (
    <View style={depth.raised} className="mx-4 mt-12 items-center rounded-3xl border border-slate-200 bg-white px-6 py-8">
      {artwork ? (
        <Illustration name={artwork} />
      ) : (
